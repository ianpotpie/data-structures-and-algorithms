...
def affine_gap_alignment(seq1, seq2, scoring_func, gap_opening_penalty, gap_extension_penalty):
    """
    This finds the global alignment of two sequences according to the affine gap scoring scheme function.
    ...
    match_mismatch_matrix = [[0 for j in range(1 + len(seq2))] for i in range(1 + len(seq1))]
    seq1_indel_matrix = [[0 for j in range(1 + len(seq2))] for i in range(1 + len(seq1))]
    for j in range(1 + len(seq2)):
        match_mismatch_matrix[0][j] = -float("inf")
    seq2_indel_matrix = [[0 for j in range(1 + len(seq2))] for i in range(1 + len(seq1))]
    ...
    for i in range(0, len(seq1)):
        for j in range(0, len(seq2)):
            ...
            optimal_score = max(indel1_score, indel2_score, match_score, 0)
            best_score_matrix[i + 1][j + 1] = optimal_score
...
def main():
    seq1 = sys.argv[1]
    seq2 = sys.argv[2]
    alignment1, alignment2, alignment_score = affine_gap_alignment(seq1, seq2, lambda x, y: 1 if x == y else -1)
